    ...
    const signInFailed = () => {
        ...
        return (
            ...
            <div className={'flex justify-center w-full'}>
                ...
                {/*Login Sheet*/}
                <div className={`${!accExist ? 'hidden' : ''} rounded-2xl z-10 mt-30 ml-auto mr-auto absolute flex flex-col justify-center items-center  min-w-1/4 max-w-full h-1/2 bg-white/10`}>
            ...
            <motion.div className={`${loading ? 'animate-pulse duration-100' : ''} bg-linear-to-bl from-blue-300/50 to-cyan-300/50 border-2 border-gray-300 h-dvh w-full flex items-center justify-center overflow-x-scroll`}>
                ...
                <div className={`${showFavorite ? 'opacity-95' : 'hidden'} overflow-auto opacity-0 absolute top-18 right-0 mr-2 mt-2 text-center h-3/4 w-1/4 z-10 bg-white/10 rounded-2xl transition-all`}><h1 className={'bg-slate-200 text-xl'}>Favorites</h1>
                <div className={''}>
                ...
                <motion.div layout transition={{duration: 2, type: "spring", stiffness: 50}}
                            className={`${!main ? 'hidden' : ''} relative border border-white/20 bg-white/30 h-125 min-w-97 max-w-full overflow-auto rounded-md`}>
                    ...
                    <div className={`${showHistory ? 'opacity-100' : 'hidden'} rounded-2xl opacity-0 absolute transition duration-300 border max-w-fit max-h-100 overflow-auto text-center bg-slate-200 z-60`}>
                        ...
                        {catalog.filter((key) =>  key.first_name === session.user.user_metadata.first_name).map((key) => (
                            <div key={key.id} className={'flex flex-col overflow-auto bg-cyan-50/80'}>
                    ...
                    {imageUrl && (
                        <div className={'mt-10 flex items-center justify-center'}>
                            <img ref={imgR} crossOrigin={'anonymous'} onLoad={imageLoad}  className={'top-20 w-65 border-2 rounded-sm max-w-2/4 transition-all duration-300 hover:-translate-y-2'} src={URL.createObjectURL(imageUrl)}/>
                        </div>
                    )}
                ...
                <div className={`${!main ? 'hidden' : ''} border-slate-100 w-1/3 h-1/2`}>
                    <div className={`h-1/2 flex items-center justify-center rounded-lg border-2`} style={{ backgroundColor: `hsl(${orgH}, ${orgS}%, ${orgL}%, ${alpha})`}}>
