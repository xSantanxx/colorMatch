    ...
    const signInFailed = () => {
        ...
        return (
            ...
            <div className={'flex justify-center w-full'}>
                ...
                <div className={`${!visible ? 'hidden' : ''} min-w-2/3 rounded-2xl z-10 mt-30 ml-auto mr-auto absolute flex flex-col justify-center items-center shadow-2xl backdrop-blur-md h-1/2 bg-white/10`}>
                    {/*Sign Up Sheet*/}
                ...
                <div className={`${!cColl ? 'hidden' : ''} transition duration-300 grid grid-cols-2 gap-2 max-w-3/4 p-2 h-screen overflow-auto`}>
                    {catalog.map((key) => (
                        ...
                        style={{'--hover-color': `hsl(${key.hue}, ${key.saturation}%, ${key.lightness}%)`}}>
                            ...
                            <img className={`${key.id === id ? 'h-83' : ''} object-contain max-h-120 max-w-full cursor-pointer`} src={`${key.image_url}`}  />
